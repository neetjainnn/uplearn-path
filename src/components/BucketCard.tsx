...
const BucketCard = ({ bucket }: { bucket: Bucket }) => (
  <div className="group rounded-lg bg-card p-6 shadow-card border border-border transition-all duration-200 hover:shadow-lg hover:-translate-y-1 hover:border-primary/30">
    ...
    <p className="text-sm text-body-text mb-4 line-clamp-2">{bucket.description}</p>
    <div className="flex flex-wrap gap-2 mb-4">
      ...
      <span className="rounded-pill bg-muted px-2.5 py-0.5 text-xs font-medium text-muted-foreground flex items-center gap-1"><Clock className="h-3 w-3" />{bucket.hours} hrs</span>
      <span className="rounded-pill bg-muted px-2.5 py-0.5 text-xs font-medium text-muted-foreground flex items-center gap-1"><BookOpen className="h-3 w-3" />{bucket.courses} courses</span>
    </div>
    <div className="flex items-center justify-between">
      <div>
        <span className="text-lg font-bold text-foreground">₹{bucket.price.toLocaleString("en-IN")}</span>
        <span className="text-xs text-muted-foreground ml-2">or subscribe</span>
      </div>
